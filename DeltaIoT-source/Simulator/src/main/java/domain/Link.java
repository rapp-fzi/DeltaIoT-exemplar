...
public class Link {
    ...
    // Constructor
    Link(RandomGenerator randomGenerator, Node from, Node to, Gateway direction, int powerNumber, int distribution) {
    ...
    void sendPacket(Packet packet, RunInfo runInfo) {
        ...
        if (randomGenerator.nextDouble() * 100 + 1 > packetLoss) {
    ...
    public double getSRN(RunInfo runInfo) { // TODO Perhaps better name 'calculateSNR'?
        ...
        double fromInt = from.getInterference()
            .get(runNumber);
        double toInt = to.getInterference()
            .get(runNumber);
        double globalInt = runInfo.getGlobalInterference()
            .get(runNumber);
